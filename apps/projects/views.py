...
from .serializers import ProjectsModelSerializer, ProjectsNamesModelSerializer, \
    InterfacesByProjectIdModelSerializer, InterfacesByProjectIdModelSerializer1,ProjectsRunSerializer
...
class ProjectsViewSet(viewsets.ModelViewSet):
    ...
    def run(self, request, *args, **kwargs):
        ...
        for testcase_obj in runnable_testcase_obj:
            # 生成yaml用例文件
            generate_testcase_file(testcase_obj, env, testcase_dir_path)

        # 运行用例（生成报告）
        # common.run_testcase(instance, testcase_dir_path)
        return run_testcase(instance, testcase_dir_path)
